...
async def handle_execute(data: dict) -> None:
    ...
    while trades_done < one_day_minimum_trade:
        ...
        if trade_result and status in ("filled", "settled", "error"):
        ...
        if status in ("filled", "settled") and outcome in ("win", "loss") and stake > 0:
            trades_done += 1
        ...
        elif status in ("filled", "settled") and outcome == "pending":
            logger.warning(
                "trade placed but settlement unknown; mm unchanged contract=%s",
                trade_result.get("trade_id"),
            )

        logger.info(
            "attempt=%s action=%s stake=%s outcome=%s trades_done=%s/%s mm_capital=%.2f",
            result.attempt_id,
