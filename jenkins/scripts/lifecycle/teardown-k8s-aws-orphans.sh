...
HELM_RELEASES="${HELM_RELEASES:-deriv-ingress deriv-apps deriv-kafka deriv-postgres deriv-platform}"
...
teardown_helm_if_cluster_up() {
  ...
  for rel in ${HELM_RELEASES}; do
    if helm status "${rel}" -n "${HELM_NAMESPACE}" >/dev/null 2>&1; then
      log "helm uninstall ${rel} -n ${HELM_NAMESPACE}"
      helm uninstall "${rel}" -n "${HELM_NAMESPACE}" --wait --timeout 10m || true
    fi
  done
  ...
  for ing in deriv-ingress deriv-apps deriv-platform; do
...
main() {
  ...
  log "  VPC_ID=${VPC_ID}"
  log "  HELM_RELEASES=${HELM_RELEASES} HELM_NAMESPACE=${HELM_NAMESPACE}"
